...
# Paths
data_dir = 'data'; csv_file = os.path.join(data_dir,'financial_data.csv')
...
def delete_date(df,date_str):
    ts=pd.Timestamp(date_str); df=df[df['Date']!=ts].reset_index(drop=True)
    df.to_csv(csv_file,index=False); return df
 ...
 def main():
    ...
    with t1:
        ...
        with st.form('f1',clear_on_submit=True):
            ...
            mos=list(calendar.month_name)[1:]; mo=st.selectbox('Month',mos,index=datetime.date.today().month-1)
            ...
            if st.form_submit_button('Add'):
                ...
                df=save_row(st.session_state['data'],date,vals); st.session_state['data']=df; st.success('Saved')
    ...
    with t3:
        ...
        if df3.empty: st.info('No data')
            ...
            yrs=sorted(df3['Date'].dt.year.unique()); mos=list(calendar.month_name)[1:]
            c1,c2=st.columns(2)
            with c1: y1=st.selectbox('From Year',yrs); m1=st.selectbox('From Month',mos)
            with c2: y2=st.selectbox('To Year',yrs,index=len(yrs)-1); m2=st.selectbox('To Month',mos,index=len(mos)-1)
            ...
            sel['MY']=sel['Date'].dt.strftime('%b %Y'); sel.set_index('MY',inplace=True)

            # Plot including negatives
            series=st.multiselect('Series',ACCOUNT_FIELDS,ACCOUNT_FIELDS)
            if series:
                fig=go.Figure()
                for f in series: fig.add_trace(go.Scatter(x=sel.index,y=sel[f]/1e6,mode='lines+markers',name=f))
